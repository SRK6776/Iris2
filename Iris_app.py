...
@st.cache()
def prediction(estimator, sepal_length, sepal_width, petal_length, petal_width):
  species = estimator.predict([[sepal_length, sepal_width, petal_length, petal_width]])
  species = species[0]
  if species == 0:
    return "Iris-setosa"
  elif species == 1:
...
if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    st.write("Species predicted:", prediction(svc_model, s_len, s_wid, p_len, p_wid))
    score = svc_model.score(X_train, y_train)

  elif classifier =='Logistic Regression':
    st.write("Species predicted:", prediction(log_reg, s_len, s_wid, p_len, p_wid))
    score = log_reg.score(X_train, y_train)

  else:
    st.write("Species predicted:", prediction(rf_clf, s_len, s_wid, p_len, p_wid))
